...
class RuleBasedGraphTransformer : public GraphTransformer {
 public:
  ...
  const Vector<std::reference_wrapper<const RewriteRule>>* GetRewriteRulesForOpType(const std::string& op_type) const {
  ...
  const Vector<std::reference_wrapper<const RewriteRule>>* GetAnyOpRewriteRules() const {
 ...
 protected:
  ...
  common::Status ApplyRulesOnNode(Graph& graph, Node& node,
                                  const Vector<std::reference_wrapper<const RewriteRule>>& rules,
 ...
 private:
  ...
  Vector<std::unique_ptr<RewriteRule>> rules_;
  // Map that associates a node's op type with the vector of rules that are registered to be triggered for that node.
  std::unordered_map<std::string, Vector<std::reference_wrapper<const RewriteRule>>> op_type_to_rules_;
  // Rules that will be evaluated regardless of the op type of the node.
  Vector<std::reference_wrapper<const RewriteRule>> any_op_type_rules_;
