...
static void RunAttnLstmTest(
    ...
    const Vector<float>& MW_data,                // memory layer weight: [num_directions, memory_depth,  am_attn_depth]
    const Vector<float>& QW_data,                // query layer weight: [num_directions, query_layer_depth=cell_hidden_size,  am_attn_depth]
    const Vector<float>& attn_v_data,            // [num_directions, am_attn_depth]
    const Vector<float>& M_data,                 // memory sequence: [batch_size, maxMemoryTimeStep, memory_depth]
    const Vector<int>* memory_sequence_lengths,  // [batch_size]
    const Vector<float>* attn_layer_weights,     // [num_directions, cell_hidden_size + memory_depth, aw_hidden_depth]
  ...
  Vector<int64_t> X_dims = {seq_length, batch_size, x_depth};
  ...
  Vector<int64_t> QW_dims{num_directions, hidden_size, am_attn_size};
  test.AddInput<float>("QW", QW_dims, QW_data);

  Vector<int64_t> MW_dims{num_directions, memory_depth, am_attn_size};
  test.AddInput<float>("MW", MW_dims, MW_data);

  Vector<int64_t> attn_v_dims{num_directions, am_attn_size};
  test.AddInput<float>("V", attn_v_dims, attn_v_data);

  Vector<int64_t> M_dims{batch_size, memory_max_time, memory_depth};
  test.AddInput<float>("M", M_dims, M_data);

  if (memory_sequence_lengths) {
    Vector<int64_t> M_seq_dims{batch_size};
  ...
  if (attn_layer_weights) {
    Vector<int64_t> attn_layer_weight_dims{num_directions, memory_depth + hidden_size, aw_attn_size};
...
template <typename T>
static Vector<T> ConcatDup(const Vector<T>& src) {
  Vector<T> dst(2 * src.size());
...
template <typename T>
static Vector<T> ConcatLastDim(const Vector<T>& a, const Vector<T>& b, int depth) {
  Vector<T> dst(2 * a.size());
...
template <typename T>
static Vector<T> Transpose2D(const Vector<T>& src, int num_rows, int num_cols) {
...
template <typename T>
static Vector<T> ConvertBatchSeqToSeqBatch(
    const Vector<T>& bs, int batch_size, int max_step, int depth) {
  Vector<T> sb(bs.size());
  int pos = 0;
...
template <typename T>
static Vector<T> ConvertIcfoToIofc(const Vector<T>& icfo, int cell_hidden_size) {
  Vector<T> iofc(icfo.size());
...
static Vector<float> s_M_data{0.1f, -0.25f, 1.0f, 1.0f, -1.0f, -1.5f, 1.0f, 0.25f, -0.125f};
static const Vector<float> s_M_2batch{0.1f, -0.25f, 1.0f, 1.0f, -1.0f, -1.5f, 1.0f, 0.25f, -0.125f,
                                           0.1f, -0.25f, 0.5f, -0.25f, -1.25f, 0.25f, -1.0f, 1.5f, -1.25f};

//real seq lens for memory
static Vector<int> s_mem_seq_lenghts{3};
static const Vector<int> s_mem_seq_lenghts_2batch{3, 2};

// [batch_size=1, input_max_step=3, input_only_depth=3]
static Vector<float> s_X_T_data{
    0.25f,
    -1.5f,
    1.0f,
    0.25f,
...
//real seq lens for X
static Vector<int> s_seq_lengths{3};

// [num_directions, memory_depth=3, am_attn_size=2]
static Vector<float> s_memory_layer_weight{4.0f, 2.0f, 0.5f, -8.0f, -2.0f, -2.0f};

// [num_directions, query_depth(cell_hidden_size)=3, am_attn_size=2]
static Vector<float> s_query_layer_weight{-0.125f, -0.25f, 0.1f, -0.125f, -0.5f, 1.5f};

// [num_directions, memory_depth+cell_hidden_size=3+3=6, aw_attn_size=2]
static Vector<float> s_attn_layer_weight{1.5f, 1.0f, 0.1f, -0.25f, 0.1f, 1.0f, -0.25f, -0.125f, -1.5f, -1.5f, -0.25f, 1.5f};

//[2]
static Vector<float> s_attn_v{-0.25f, 0.1f};

//lstm kernel weights, [8, 12]  8 = x_depth + aw_attn_size + cell_hidden_size, 12 = 4 * cell_hidden_size (ijfo)
static Vector<float> s_WR_T_data_ICFO{
...
Vector<float> s_lstm_cell_bias_ICFO{
...
TEST(AttnLSTMTest, ForwardLstmWithBahdanauAMZeroAttention) {
  Vector<float> X_data = ConvertBatchSeqToSeqBatch(s_X_T_data, batch_size, input_max_step, input_only_depth);

  //clear attention layer weight, so that no attention value will be in effective
  Vector<float> zero_attn_layer_weight(s_attn_layer_weight.size(), 0.0f);

  Vector<float> WR_T_data = ConvertIcfoToIofc(s_WR_T_data_ICFO, cell_hidden_size);

  const size_t W_data_size = 5 * 12;
  Vector<float> W_T_data(&(WR_T_data[0]), &(WR_T_data[0]) + W_data_size);
  // Fake zero for attention input weight now
  std::fill(W_T_data.begin() + 3 * 12, W_T_data.begin() + W_data_size, 0.0f);

  Vector<float> R_T_data(&(WR_T_data[0]) + W_data_size, &(WR_T_data[0]) + WR_T_data.size());

  // transpose W and R for onnx sematic
  Vector<float> W_data = Transpose2D(W_T_data, input_size, 4 * cell_hidden_size);
  Vector<float> R_data = Transpose2D(R_T_data, cell_hidden_size, 4 * cell_hidden_size);

  Vector<float> B_data = ConvertIcfoToIofc(s_lstm_cell_bias_ICFO, cell_hidden_size);

  // [1, 3, 3]
  Vector<float> Y_T_data{
  ...
  Vector<float> Y_data = ConvertBatchSeqToSeqBatch(
...
TEST(AttnLSTMTest, BidirectionLstmWithBahdanauAMShortenSeqLength) {
  ...
  Vector<float> Y_data = ConcatLastDim(
...
TEST(AttnLSTMTest, BidirectionLstmWithBahdanauAM2BatchShortenSeqLen) {
  ...
  static const Vector<float> s_X_T_2batch{0.25f, -1.5f, 1.0f, 0.25f, -0.5f, -1.5f, 0.1f, 1.5f, 0.25f, 0.0f, 0.0f, 0.0f,
                                               0.1f, -0.125f, 0.25f, -0.5f, 0.25f, 0.1f, 1.0f, 0.5f, -1.5f, 0.0f, 0.0f, 0.0f};
  static const Vector<int> s_seq_lengths_2batch{3, 2};

  Vector<float> X_data = ConvertBatchSeqToSeqBatch(s_X_T_2batch, batch2Size, inputMaxStep4, input_only_depth);
