...
static void RunAttentionTest(
    const Vector<float>& input_data,         // input:      [batch_size, sequence_length, hidden_size]
    const Vector<float>& weights_data,       // weights:    [hidden_size, 3 * hidden_size]
    const Vector<float>& bias_data,          // bias:       [3 * hidden_size]
    const Vector<int32_t>& mask_index_data,  // mask_index: [batch_size]
    const Vector<float>& output_data,        // output:     [batch_size, sequence_length, hidden_size]
    int batch_size,
    int sequence_length,
    int hidden_size,
    int number_of_heads,
  ...
  if (enable_cpu || enable_cuda) {
    ...
    Vector<int64_t> weights_dims = {hidden_size, 3 * hidden_size};
    Vector<int64_t> bias_dims = {3 * hidden_size};
...
TEST(AttentionTest, AttentionBatch1) {
  ...
  Vector<int32_t> mask_index_data = {2L};
...
TEST(AttentionTest, AttentionBatch2) {
  ...
  Vector<int32_t> mask_index_data = {2L, 2L};
...
TEST(AttentionTest, AttentionMaskPartialSequence) {
  ...
  Vector<int32_t> mask_index_data = {1L};
...
TEST(AttentionTest, AttentionMaskExceedSequence) {
  ...
  // Test mask_index > sequence_length
  Vector<int32_t> mask_index_data = {3L};
