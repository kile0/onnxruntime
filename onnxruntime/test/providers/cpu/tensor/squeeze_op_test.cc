...
TEST(SqueezeOpTest, Squeeze_Empty_Axes_1) {
  OpTester test("Squeeze");
  test.AddInput<float>("data", {1, 1, 4, 1}, Vector<float>(4, 1.0f));
  test.AddOutput<float>("squeezed", {4}, Vector<float>(4, 1.0f));
...
TEST(SqueezeOpTest, Squeeze_Empty_Axes_2) {
  ...
  test.AddInput<float>("data", {2, 4}, Vector<float>(8, 1.0f));
  test.AddOutput<float>("squeezed", {2, 4}, Vector<float>(8, 1.0f));
  // TensorRT doesn't seem to support missing 'axes'
  test.Run(OpTester::ExpectResult::kExpectSuccess, "", {kTensorrtExecutionProvider});
}
