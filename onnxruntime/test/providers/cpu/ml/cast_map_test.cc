...
static void RunTest(const std::map<int64_t, TFrom>& input,
  ...
  Vector<int64_t> dims{1, gsl::narrow_cast<int64_t>(output.size())};
...
TEST(CastMap, DenseStringToFloat) {
  ...
  Vector<float> output{-1.0f, 1.0f, 2.0f, -3.0f};
...
TEST(CastMap, SparseFloatToFloat) {
  ...
  Vector<float> output{0.0f, 1.0f, 2.0f, 0.0f};
...
TEST(CastMap, StringToString) {
  ...
  Vector<std::string> output{"-1.0f", "3"};
...
TEST(CastMap, FloatToString) {
  ...
  Vector<std::string> output{"-1.000000", "3.000000"};
