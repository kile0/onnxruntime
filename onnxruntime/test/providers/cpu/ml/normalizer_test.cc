...
TEST(Normalizer, SingleDimensionFloat) {
  ...
  Vector<float> input = {-1.f, 0.f, 1.f};

  Vector<float> max_output{-1.0f, 0.f, 1.0f};
  Vector<float> l1_output{-0.5f, 0.f, 0.5f};
  Vector<float> l2_output{-0.70710677f, 0.f, 0.70710677f};
...
TEST(Normalizer, TwoDimensionDouble) {
  ...
  Vector<double> input = {-1.0856306, 0.99734545, 0.2829785,
                               -1.50629471, -0.57860025f, 1.65143654};

  Vector<float> max_output{-1.0885202f, 1.f, 0.2837317f,
                                -0.91211176f, -0.35036176f, 1.f};

  Vector<float> l1_output{-0.45885524f, 0.42154038f, 0.11960436f,
                               -0.40314806f, -0.15485784f, 0.44199413f};

  Vector<float> l2_output{-0.7232126f, 0.6643998f, 0.18851127f,
...
TEST(Normalizer, ThreeDimensionInt32) {
  ...
  Vector<int32_t> input = {-242, -42, 126, -86,
                                -67, -9, 149, -63,
                                -44, -43, 220, 218,
  ...
  Vector<float> max_output{5.5f, 4.6666665f, 0.57272726f, -0.39449543f,
  ...
  Vector<float> l1_output{-0.6855524f, -0.44680852f, 0.25454545f, -0.23433243f,
  ...
  Vector<float> l2_output{-0.94928247f, -0.6910363f, 0.4284698f, -0.3543899f,
...
TEST(Normalizer, InvalidNorm) {
  Vector<int64_t> dims = {3};
  Vector<float> input = {-1.f, 0.f, 1.f};
  Vector<float> output{-1.0f, 0.f, 1.0f};
